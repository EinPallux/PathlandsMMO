...
- **XP-source split (flagged by the Phase-4 acceptance pass):** §5 targets quests ~55% / kills ~35% / discovery ~10%, but the current numbers make **kills the dominant source** — the level curve is `400·L^1.55` (~878k XP to cap) while all authored quest rewards sum to ~32k (≈4%). Reachability is fine (kills are unbounded and quests blanket the whole 1→30 band with no dead zone, so there is no grinding _wall_), but the _feel_ leans grindier than §5 intends. Phase-5 tuning must reconcile the two: either raise quest XP toward the ~55% share (roughly a 5–10× lift, and/or fill the side-quest budget toward ~110) or restate §5's split to match a kill-led economy. Do not change the curve without re-checking the 25–35 h target above.
