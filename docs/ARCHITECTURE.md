...
- **Save versioning:** `SAVE_VERSION` 6 — v2→v3 added the quest log; v3→v4 added profession skills + the material stash; v4→v5 added the crafted-consumables stash; v5→v6 added meta progression (`deeds`, `pathPoints`, `perks`). `migrate()` walks any prior shape forward.
