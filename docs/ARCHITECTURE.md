...
- **Save versioning:** `SAVE_VERSION` 9 — v2→v3 added the quest log; v3→v4 added profession skills + the material stash; v4→v5 added the crafted-consumables stash; v5→v6 added meta progression (`deeds`, `pathPoints`, `perks`); v6→v7 added mounts (`mounts`, `activeMount`); v7→v8 added the bank vault + mail inbox (`bank`, `mail`); v8→v9 added the daily-bounty log (`bounties`). `migrate()` walks any prior shape forward.
