...
- **Content is data (`shared/data`):** enemies/skills/items/loot, the **world spawn table** (`spawns.ts` — one region list for zones + Hollow packs + bosses, referencing the settlement/Hollow coords), **boss encounter scripts** (`EnemyDef.boss` — HP-threshold beats interpreted by `stepBossMechanics`), and **vendor** stock/pricing (`vendors.ts`). The client driver activates spawn regions by player proximity (culling distant enemies); in P6 the server owns spawning and boss scripts unchanged.
