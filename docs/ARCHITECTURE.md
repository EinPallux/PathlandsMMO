...
- **Quests (`shared/quests`):** a pure state machine over the quest definitions — `acceptQuest` → `applyQuestEvent` (advances objectives from kills/exploration/talks/world-object use) → `turnInQuest` (returns the reward to grant). The client's `QuestDirector` feeds it events and grants rewards; P6 moves the log server-side unchanged. Quest state is part of the versioned save (`SAVE_VERSION` 3: v2→v3 added the quest log).
